...
/*
   ...
   71. get_partner_permission($partner_id, $permit = NULL)
...
function get_settings($type = 'system_settings', $is_json = false)
{
    ...
    if (isset($cached_settings[$type])) {
        return $cached_settings[$type][$is_json ? 'json' : 'raw'];
    }
    ...
    if (!empty($res)) {
        $raw = output_escaping($res[0]['value']);
        $json = json_decode($res[0]['value'], true);

        // Cache both raw and JSON values
        $cached_settings[$type] = [
            'raw' => $raw,
            'json' => $json
        ];

        return $is_json ? $json : $raw;
    }

    return $is_json ? [] : null; // fallback return
}
...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $partner_id = NULL, $sort_by = "pd.user_id")
{
    ...
    if (isset($filter['show_only_active_products']) && $filter['show_only_active_products'] == 0) {
        ...
        $where = ['p.status' => 1, 'pv.status' => 1, 'pd.status' => 1];
    }

    if (isset($filter['currently_available']) && $filter['currently_available'] == 0) {
        $where = [];
    } else {
        if (isset($filter['show_only_active_products'])) {

            if ($filter['show_only_active_products'] == 'false') {
    ...
    if (isset($filter) && !empty($filter['search'])) {
        ...
        $t->db->group_Start();
        $t->db->like('p.name', trim($filter['search']));
        $t->db->or_like('pd.partner_name', trim($filter['search']));
    ...
    if (isset($filter) && !empty($filter['tags'])) {
        ...
        $t->db->join('`partner_tags` rt', 'rt.partner_id = pd.user_id', 'LEFT');
    ...
    if (isset($filter) && $filter['vegetarian'] != "") {
        if ($filter['vegetarian'] == "3") {
            // $t->db->where('pd.type ', $filter['vegetarian']);
        } else {
            if ($sort_by == "p.id") {
                $t->db->where('p.indicator ', $filter['vegetarian']);
            }
            if ($sort_by == "pd.user_id") {
                $t->db->where('pd.type ', $filter['vegetarian']);
            }
        }
    }
    ...
    if (isset($filter) && !empty($filter['partner_slug']) && $filter['partner_slug'] != "") {
        $where['pd.slug'] = $filter['partner_slug'];
    }
    //category slug
    ...
    if (!empty($return_count)) {
        ...
        $t->db->where('pd.status ', 1);
        $t->db->where('pd.licence_status ', 1);
    ...
    if (isset($filter) && !empty($filter['search'])) {
        ...
        $product_count->or_like('pd.partner_name', $filter['search']);
        $t->db->group_End();
    }
    ...
    if (isset($filter) && $filter['vegetarian'] != "") {
        if ($filter['vegetarian'] == "3") {
            ...
            $t->db->where('p.indicator ', $filter['vegetarian']);
            $t->db->where_in('pd.type ', [$filter['vegetarian'], 3]);
        }
    }
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            if(!empty(get_variants_values_by_pid($product[$i]['id']))){
                $is_best_seller = "0"; // Assume not a best seller
                  ...
                  if (isset($filter) && !empty($filter['city_id']) && !empty($filter['city_id'])) {
                      $restro_filter['city_id'] = $filter['city_id'];
                  }
                  $user_id_restro = "";
                ...
                //   print_r($restro_filter);
                      ...
                      for ($k = 0; $k < count($product[$i]['variants']); $k++) {
                          ...
                          if (isset($user_id) && $user_id != NULL) {
                              ...
                              if (!empty($user_cart_data)) {
                                  $product[$i]['variants'][$k]['cart_count'] = isset($user_cart_data['quantity']) && !empty($user_cart_data['quantity']) ? $user_cart_data['quantity'] : "0";
                              } else {
                  ...
                  $variant_attributes = [];
                  $attributes_array = isset($product[$i]['variants'][0]['attr_name']) ? explode(',', $product[$i]['variants'][0]['attr_name']) : [];
...
//validate_promo_code
function validate_promo_code($promo_code, $user_id, $final_total, $wallet_balance_used, $partner_id = NULL)
{
    if (isset($promo_code) && !empty($promo_code)) {
        ...
        $promo_code = $t->db->select('pc.*,count(o.id) as promo_used_counter ,( SELECT count(user_id) from orders where user_id =' . $user_id . ' and promo_code ="' . $promo_code . '") as user_promo_usage_counter ')
            ...
            ->or_where('pc.partner_id',$partner_id)
        ...
        if (!empty($promo_code[0]['id'])) {
            ...
            if (intval($promo_code[0]['promo_used_counter']) < intval($promo_code[0]['no_of_users'])) {
                ...
                if ($final_total >= intval($promo_code[0]['minimum_order_amount']) || $wallet_balance_used >= intval($promo_code[0]['minimum_order_amount'])) {
...
function update_wallet_balance($operation, $user_id, $amount, $message = "Balance Debited", $status = NULL)
{
...
// Generate Access Token for push notification
...
{
    $t = &get_instance();
    $t->db->select('(select sum(c.qty)  from cart c join product_variants pv on c.product_variant_id=pv.id join products p on p.id=pv.product_id join partner_data pd on pd.user_id=p.partner_id  where c.user_id="' . $user_id . '" and qty!=0  and  is_saved_for_later = "' . $is_saved_for_later . '" and p.status=1 AND pv.status=1 AND pd.status=1) as total_items,(select count(c.id) from cart c join product_variants pv on c.product_variant_id=pv.id join products p on p.id=pv.product_id join partner_data pd on pd.user_id=p.partner_id where c.user_id="' . $user_id . '" and qty!=0 and  is_saved_for_later = "' . $is_saved_for_later . '" and p.status=1 AND pv.status=1 AND pd.status=1) as cart_count,`c`.qty,p.is_prices_inclusive_tax,p.cod_allowed,p.is_spicy,p.minimum_order_quantity,p.slug,p.total_allowed_quantity, p.name, p.image,p.short_description,`c`.user_id,pv.*,tax.percentage as tax_percentage,tax.title as tax_title');
    ...
    $t->db->join('products p ', 'pv.product_id=p.id');
    $t->db->join('partner_data pd ', 'pd.user_id=p.partner_id');
    $t->db->join('`taxes` tax', 'tax.id = p.tax', 'LEFT');
    $t->db->join('categories ctg', 'p.category_id = ctg.id', 'left');
    $t->db->where(['p.status' => '1', 'pv.status' => 1, 'pd.status' => 1]);
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $rider_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $download_thermal_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $partner_id = null, $is_pending = false, $rider_city_id = NULL)
{
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        $order_details[$i]['name'] = isset($order_details[$i]['name']) ? stripslashes($order_details[$i]['name']) : "";
        $t->db->select('oi.*,p.id as product_id,p.is_cancelable,p.cancelable_till,p.partner_id as partner_id ,p.is_returnable,p.image,p.name,p.indicator,p.type,(Select count(id) from order_items where order_id = oi.order_id ) as order_counter')
            ->join('product_variants pv', 'pv.id=oi.product_variant_id', "left")
            ->join('products p', 'pv.product_id=p.id', 'left')
        ...
        if (isset($order_details[$i]['rider_id']) && !empty($order_details[$i]['rider_id'])) {
            ...
            $order_details[$i]['order_rider_rating'] = !empty($oredr_wise_rider_rating) ? $oredr_wise_rider_rating[0]['rating'] : "";
        ...
        for ($k = 0; $k < count($order_item_data); $k++) {
            unset($order_item_data[$k]['partner_detail_snapshot']);
            if (!empty($order_item_data)) {
                ...
                if (!empty($restro_data) && !empty($restro_data['data'])) {
                        ...
                        $order_item_data[$k]['partner_details'] = json_decode($order_item_data[$k]['partner_details'],true);
                }
                ...
                $order_item_data[$k]['product_name'] = (!empty($order_item_data[$k]['name'])) ? stripslashes($order_item_data[$k]['name']) : stripslashes($order_item_data[$k]['product_name']);
                $order_item_data[$k]['name'] = (!empty($order_item_data[$k]['name'])) ? stripslashes($order_item_data[$k]['name']) : stripslashes($order_item_data[$k]['product_name']);
...
{
    $t = &get_instance();
    $user_details = $t->db->select('u.id,username,email,u.mobile,u.type,balance,dob,u.image,referral_code, friends_code, c.name as city_name,a.area as area,a.landmark,a.pincode')
    ...
    if (!empty($user_details)) {
        ...
        $user_details = []; // or handle it however you'd prefer, like returning a specific value
    }
...
function get_favorites($user_id, $type = 'products', $limit = NULL, $offset = NULL)
{
    ...
    if ($type == 'products') {
        ...
        if (!empty($res)) {
            ...
            $pro_details = fetch_product($user_id, null, $product_ids, "", "", "", "", "", "", "", "", "p.id");
        ...
        $q = $CI->db->select("f.*, u.username as owner_name, p.user_id, p.cooking_time, p.slug, p.partner_name, p.type, p.store_url, p.description, p.profile, p.address, p.rating as partner_rating, p.no_of_ratings, p.cooking_time as partner_cook_time, 
            (SELECT MIN(IF(pv.special_price != 0, pv.special_price, pv.price)) 
            FROM products pr 
            JOIN product_variants pv ON pv.product_id = pr.id 
            WHERE pr.partner_id = p.user_id) as price_for_one")
        ...
        if (isset($favorites) && !empty($favorites)) {
            if (isset($favorites) && !empty($favorites)) {
                for ($i = 0; $i < count($favorites); $i++) {
                    ...
                    $favorites[$i]['price_for_one'] = $favorites[$i]['price_for_one'];
...
function process_refund($id, $status, $type = 'orders')
{
    ...
    $user_res = fetch_details(['id' => $user_id], 'users', 'fcm_id,web_fcm_id,platform');
    $fcm_ids = array();
    ...
    if ($payment_method != 'cod') {
        ...
        if ($is_delivery_charge_returnable == 1) {
            ...
            $returnable_amount =  $order_details[0]['final_total'];
        }
...
function get_partner_permission($partner_id, $permit = NULL)
{
    $permits = fetch_details(['user_id' => $partner_id], 'partner_data', 'permissions');
    if (!empty($permit)) {
        if (!empty($permits)) {
...
function get_price($type = "max")
{
    ...
    $t->db->select('IF( pv.special_price > 0, `pv`.`special_price`, pv.price ) as pr_price')
        ...
        ->join('`product_variants` pv', 'p.id = pv.product_id', 'LEFT')
        ->join('`product_attributes` pa', ' pa.product_id = p.id ', 'LEFT');
    $t->db->where(" `p`.`status` = '1' AND `pv`.`status` = 1 AND `pd`.`status` = 1 AND   (`c`.`status` = '1' OR `c`.`status` = '0')");
...
{
 $global_time = fetch_details(['user_id' => $id],'partner_data','global_restaurant_time')[0]; 
 if($global_time['global_restaurant_time'] == 1){
...
//     // Fetch partner location details
//     $partner_data = fetch_details(['id' => $partner_id], "users", "latitude,longitude,city");
//     if (empty($partner_data)) {
//         return false;
//     }

//     // Determine the city for deliverability check
//     $city_id = ($type == "city") ? $address_id : $partner_data[0]['city'];

//     // Fetch city geolocation details
//     $partner = fetch_details(['id' => $city_id], 'cities', 'geolocation_type,radius,boundary_points,max_deliverable_distance');
//     if (empty($partner)) {
//         return false;
//     }

//     // Calculate distance between points
//     $distance = calculateDistanceForCircle(
//         $partner_data[0]['latitude'],
//         $partner_data[0]['longitude'],
//         $latitude_y,
//         $longitude_x
//     );

//     // Check deliverability based on geolocation type
//     switch ($partner[0]['geolocation_type']) {
//         case "polygon":
//             // Decode boundary points
//             $boundary_points = json_decode($partner[0]['boundary_points'], true);
//             $vertices_x = array_column($boundary_points, "lng");
//             $vertices_y = array_column($boundary_points, "lat");
//             $points_polygon = count($vertices_x);

//             // Check if point is within polygon and within max distance
//             return (
//                 is_in_polygon($points_polygon, $vertices_x, $vertices_y, $longitude_x, $latitude_y) &&
//                 $distance <= $partner[0]['max_deliverable_distance']
//             );

//         case "circle":
//             // Check if point is within max distance
//             return $distance <= $partner[0]['max_deliverable_distance'];
...
function calculateDistanceForCircle($lat1, $lon1, $lat2, $lon2)
{
...
function fetch_partners($filter = [], $user_id = null, $limit = NULL, $offset = '', $sort = 'u.id', $order = 'DESC', $search = NULL)
{
    // print_r($filter);
    if (isset($filter) && !empty($filter['latitude']) && !empty($filter['longitude'])) {
        $max_distance = fetch_details(['id' => $filter['city_id']], "cities", "max_deliverable_distance");
    ...
    $search_res = $t->db->select($where_near_by_for_select . ' `u`.username as owner_name,
    (SELECT MIN(IF(pv.special_price != 0, pv.special_price, pv.price)) 
     FROM `products` p 
     JOIN `product_variants` pv ON pv.product_id = p.id 
     WHERE p.partner_id = u.id) as price_for_one, 
     u.id as partner_id, 
     u.email, u.mobile, u.balance, 
     pd.address as partner_address, 
     u.city as city_id, 
     c.name as city_name, 
     c.time_to_travel, 
     u.fcm_id, u.latitude, u.longitude, 
     `pd`.* ')
    ->join('users_groups ug', ' ug.user_id = u.id ')
    ->join('partner_data pd', ' pd.user_id = u.id ')
    ->join('products p', ' p.partner_id = u.id ', "left")
    ...
    foreach ($restro_search_res as $row) {
        ...
        if (isset($row['licence_proof']) && !empty($row['licence_proof'])) {
            $licence_proof = json_decode($row['licence_proof']);
            $licence_proof = array_map(function ($value) {
                return  base_url() . $value;
            }, $licence_proof);
        } else {
            $licence_proof = [];
        ...
        $tempRow['price_for_one'] = (isset($row['price_for_one']) && !empty($row['price_for_one'])) ? $row['price_for_one'] : "no price set";
        ...
        $permit = json_decode($row['permissions'], true);
        $permmissions = array();
        $permmissions['customer_privacy'] = (isset($permit['customer_privacy']) && !empty($permit['customer_privacy'])) ? $permit['customer_privacy'] : "0";
        $permmissions['view_order_otp'] = (isset($permit['view_order_otp']) && !empty($permit['view_order_otp'])) ? $permit['view_order_otp'] : "0";
        $permmissions['assign_rider'] = (isset($permit['assign_rider']) && !empty($permit['assign_rider'])) ? $permit['assign_rider'] : "0";
        $permmissions['is_email_setting_on'] = (isset($permit['is_email_setting_on']) && !empty($permit['is_email_setting_on'])) ? $permit['is_email_setting_on'] : "0";
        $permmissions['delivery_orders'] = (isset($permit['delivery_orders']) && !empty($permit['delivery_orders'])) ? $permit['delivery_orders'] : "0";
        $permmissions['self_pickup'] = (isset($permit['self_pickup']) && !empty($permit['self_pickup'])) ? $permit['self_pickup'] : "0";
        $permmissions['partner_wise_promocode'] = (isset($permit['partner_wise_promocode']) && !empty($permit['partner_wise_promocode'])) ? $permit['partner_wise_promocode'] : "0";
        $tempRow['permissions'] = (isset($permmissions) && !empty($permmissions)) ? $permmissions : "";
        ...
        $tempRow['global_restaurant_time'] = $row['global_restaurant_time'];
...
function send_notifications($role_id = "", $role = "user", $title = "", $body = "", $type = "", $id = "")
{
    ...
    if ($role == 'admins') {
        ...
        $partner = fetch_details(['u.id' => $role_id], 'users u', 'u.email,rd.partner_name,u.fcm_id,u.web_fcm_id,u.platform', null, null, null, "DESC", "", '', "partner_data rd", "rd.user_id=u.id");


        if (!empty($partner[0]['fcm_id'])) {
            // Step 1: Group by platform
            $groupedByPlatform = [];
            ...
            $fcmMsg = array(
                'title' => $title,
                'body' => 'Hello Dear ' . $partner[0]['username'] . "\n" . $body,
                'type' => (string)$type,
                'type_id' => (string)$id,
            );
        ...
        if (get_partner_permission($role_id, 'is_email_setting_on')) {
            send_mail($partner[0]['email'], $title,$body);
        }
        return true;
    } else if ($role == "user") {
        $user_res = fetch_details(['id' => $role_id], 'users', 'username,web_fcm_id,fcm_id,platform,email');
        ...
        return true;
         ...
         $riders  = $t->db->select("u.serviceable_city,u.fcm_id,u.web_fcm_id,u.platform,u.username")
        ...
        foreach ($riders as $key => $rider) {
            ...
            //     );
            //     if(!empty($rider['fcm_id'])){

            //         $fcm_ids[0][] = $rider['fcm_id'];
            //     }
            //     if(!empty($rider['web_fcm_id'])){
...
function update_rider($rider_id, $order_id, $status = "accepted")
{
    ...
    if (!empty($user_res[0]['fcm_id'])) {
        ...
        $fcm_ids = [];
        foreach ($groupedByPlatform as $platform => $fcmIds) {
            $fcm_ids[$platform] = array_chunk($fcmIds,
                1000
            );
        }
        $fcm_ids_test = $fcm_ids;
...
function get_partners_option_html($partners, $selected_vals = null)
{
    $html = "";
    for ($i = 0; $i < count($partners); $i++) {
        $pre_selected = (!empty($selected_vals) && in_array($partners[$i]['user_id'], $selected_vals)) ? "selected" : "";
        $html .= '<option value="' . $partners[$i]['user_id'] . '" ' . $pre_selected . '  >' . output_escaping($partners[$i]['partner_name']) . '</option>';
    }
    return $html;
}
