...
class Setting_model extends CI_Model
{
    ...
    public function update_web_setting($post)
    {
        ...
        $menifest_logo = (isset($post['menifest_logo']) && !empty($post['menifest_logo'])) ? $post['menifest_logo'] : "";
        ...
        if ($menifest_logo != NULL && !empty($menifest_logo)) {
            $menifest_logo_res = $this->db->get_where('settings', array(
                'variable' => 'menifest_logo'
            ));
            $menifest_logo_count = $menifest_logo_res->num_rows();
            if ($menifest_logo_count == 0) {
                $this->db->insert('settings', ['value' => $menifest_logo, 'variable' => 'menifest_logo']);
            } else {
                $this->db->set('value', $menifest_logo)->where('variable', 'menifest_logo')->update('settings');
            }
        }
        if ($count === 0) {
            $data = array(
