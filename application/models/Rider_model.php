...
class Rider_model extends CI_Model
{
    ...
    function get_riders_list($cash_collection =false, $rider_requests = false)
    {
        ...
        if (isset($where) && !empty($where)) {
            ...
            if ($rider_requests == true) {
                ...
                $count_res->group_start();
                $count_res->where('u.active', '1');
                $count_res->or_where('u.active', '0');
                $count_res->group_end();
            }
        ...
        $search_res = $this->db->select(' u.*,DATE(u.created_at) as created_at,c.name as city_name ')->join('users_groups ug', ' ug.user_id = u.id ')->join("cities c", "c.id=u.serviceable_city", "left");
        if (isset($multipleWhere) && !empty($multipleWhere)) {
        ...
        if (isset($where) && !empty($where)) {
            ...
            if ($rider_requests == true) {
                ...
                $search_res->group_start();
                $search_res->where('u.active', '1');
                $search_res->or_where('u.active', '0');
                $search_res->group_end();
            }
    ...
    public function get_riders($id, $search, $offset, $limit, $sort, $order,$partner_login = false)
    {
        $multipleWhere = '';
        $where['ug.group_id'] =  3;
        if (!empty($search)) {
