...
class Product_model extends CI_Model
{
        ...
        if ($pro_type == 'simple_product') {
            ...
            if (!empty($data['variants_ids'])) {
                ...
                if (!isset($data['edit_variant_id']) && isset($data['edit_product_id'])) {
                    $this->db->set('status', 7)->where('product_id', $data['edit_product_id'])->where('status !=', 0)->where_not_in('attribute_value_ids', $data['variants_ids'])->update('product_variants');
                }
    ...
    public function get_product_details($flag = NULL, $partner_id = NULL, $p_status = NULL)
    {
        ...
        $search_res = $this->db->select('product_variants.id AS id,pd.partner_name, p.id as pid ,p.rating,p.no_of_ratings,p.name, p.type, p.image, p.status,product_variants.price , product_variants.special_price, product_variants.stock ')
        ...
        foreach ($pro_search_res as $row) {
            ...
            // $tempRow['product_name'] = (isset($row['name']) ? stripslashes(stripslashes($row['name'])) : "") . '<br><small>' . ucwords(str_replace('_', ' ', $row['type'])) . '</small><br><small> By </small><b>' . isset($row['partner_name']) ? stripslashes($row['partner_name']) : "" . '</b>';
            $tempRow['product_name'] =
            (isset($row['name']) ? stripslashes(stripslashes($row['name'])) : "") .
            '<br><small>' . ucwords(str_replace('_', ' ', $row['type'])) . '</small>' .
            '<br><small> By </small><b>' . (isset($row['partner_name']) ? stripslashes($row['partner_name']) : "") . '</b>';

            $tempRow['name'] = isset($row['name']) ? stripslashes($row['name']) : "";
    ...
    public function get_product_add_ons($product_id = "", $from_api = false)
    {
        ...
        if (isset($_GET['search']) and $_GET['search'] != '') {
            ...
            $multipleWhere = [
                ...
                '`description`' => $search,
                '`price`' => $search,
                '`calories`' => $search  // Removed extra space
            ];
