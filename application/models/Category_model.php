...
class Category_model extends CI_Model
{
    ...
    public function get_categories($id = NULL, $limit = '', $offset = '', $sort = 'row_order', $order = 'ASC', $has_child_or_item = 'true', $slug = '', $ignore_status = '', $search = '', $partner_slug = '',$active_status = '')
    {
        ...
        if (!empty($partner_id)) {
            $sub_query = "SELECT DISTINCT(category_id) FROM `products` WHERE partner_id = $partner_id";
        ...
        $this->db->where('status', 1); // Add the same condition here
        $count_res = $this->db->count_all_results('categories c1');
        $i = 0;
