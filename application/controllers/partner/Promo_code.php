...
class Promo_code extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_partner()) {
            ...
            $partner_detail = fetch_details(['user_id' => $partner_id], 'partner_data', 'permissions');
            $partner_permissions = isset($partner_detail[0]['permissions']) ? json_decode($partner_detail[0]['permissions'], true) : "";
             if($partner_permissions['partner_wise_promocode'] != 1){
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_partner()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($endDateTime <= $startDateTime) {
                    ...
                    $_POST['partner_id'] = $this->ion_auth->get_user_id();
