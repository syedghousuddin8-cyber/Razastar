    ...
    public function create_partner()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->response['message'] = array(
                    ...
                    'licence_proof' => form_error('licence_proof'),
                );
            ...
            if (isset($_POST['user_id']) && !empty($_POST['user_id'])) {
                ...
                $user_id_to_partner = $this->input->post('user_id', true);
                ...
                if ($this->Partner_model->add_partner($partner_data)) {
                    $group_id = $this->ion_auth->get_users_groups($user_id_to_partner)->row()->id;
                    $this->ion_auth->remove_from_group($group_id, $user_id_to_partner);
                    $this->ion_auth->add_to_group('4', $user_id_to_partner);
                ...
                if (update_details(['active' => 1], [$identity_column => $identity], 'users')) {
                    ...
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = "Partner not Registared.";
                    print_r(json_encode($this->response));
                }
            }
        }
