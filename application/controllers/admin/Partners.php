...
class Partners extends CI_Controller
{
    ...
    public function manage_partner()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                ...
                $this->data['fetched_data'] = $this->db->select(' u.*,pd.* ')
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->response['messages'] = array(
                    ...
                    'licence_name' => form_error('licence_name'),
                    'licence_code' => form_error('licence_code'),
                );
                print_r(json_encode($this->response));
                return false;
                ...
                if($_POST['global_commission'] > 100){
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] =  "Admin commission can not be grater then 100 percent";
                    print_r(json_encode($this->response));
                    return;
                }
                ...
                if(isset($_POST['edit_restro'])){
                    ...
                    $delivery_type = fetch_details(['user_id' => $_POST['edit_restro']],'partner_data','permissions');
                    $delivery_permissions = isset($delivery_type) && !empty($delivery_type) ? json_decode($delivery_type[0]['permissions'],true) : "";
                }
               
                $permmissions['self_pickup'] = isset($delivery_permissions['self_pickup']) ? 1 : 0;
                $permmissions['delivery_orders'] = isset($delivery_permissions['delivery_orders']) ? 1 : 0;
                ...
                $permmissions['partner_wise_promocode'] = (isset($_POST['partner_wise_promocode'])) ? 1 : 0;
                ...
                if (isset($_POST['edit_restro'])) {
                    ...
                    if ($this->Partner_model->add_partner($restro_data, $profile, $work_time, $tags)) {
                        ...
                        $this->response['location'] = base_url('admin/partners/');
                        print_r(json_encode($this->response));
                    } else {
                        $this->response['error'] = true;
                        $this->response['csrfName'] = $this->security->get_csrf_token_name();
    ...
    public function top_partner()
    {

        $this->Partner_model->top_partners();
    }
}
