...
class Transaction extends CI_Controller {
    ...
    public function wallet_transactions()
	{
		if($this->ion_auth->logged_in() && $this->ion_auth->is_admin())
		{
            ...
            $this->data['partners'] = $this->db->select(' pd.partner_name,u.id as partner_id,pd.id as partner_data_id  ')
			...
			$partner['partner_name'] = isset($partner['partner_name']) ? stripslashes($partner['partner_name']) : "";
			}
					$this->load->view('admin/template',$this->data);
		}
		else{
	...
	public function view_order_transactions()
	{
		if($this->ion_auth->logged_in() && $this->ion_auth->is_admin())
		{			
			return $this->Transaction_model->get_transactions_list();
	...
	public function order_settelment_transaction()
	{
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			$this->data['main_page'] = TABLES . 'manage-order-transaction';
			$settings = get_settings('system_settings', true);
			$this->data['title'] = 'Manage Order Transaction | ' . $settings['app_name'];
			$this->data['meta_description'] = 'Manage Order Transaction | ' . $settings['app_name'];
			$this->load->view('admin/template', $this->data);
		} else {
			redirect('admin/login', 'refresh');
		}
