...
class Payment_statistics extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            $this->data['main_page'] = TABLES . 'manage-payment-statistics';
            $settings = get_settings('system_settings', true);
            $this->data['title'] = 'Manage Payment Statistics | ' . $settings['app_name'];
            $this->data['meta_description'] = 'Manage Payment Statistics | ' . $settings['app_name'];
    ...
    public function view_wallet_customer()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            return $this->Customer_model->get_customer_list($wallet_customer = 1);
        } else {
            redirect('admin/login', 'refresh');
        }
    }
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($_POST['amount'] < 0) {
                    ...
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = "Amount Can not be less then 0.";
                    print_r(json_encode($this->response));
                    return false;
                }
                if ($_POST['type'] == 'debit' || $_POST['type'] == 'credit') {
                    $response = update_wallet_balance($_POST['type'], $_POST['user_id'], $_POST['amount'], $_POST['message'], $_POST['status']);
