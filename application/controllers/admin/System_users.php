...
class System_users extends CI_Controller
{
	...
	public function update_system_user()
	{
		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			// $this->form_validation->set_rules('email', 'Email', 'trim|required|xss_clean|valid_email|is_unique[users.email]', array('is_unique' => ' The email is already registered.'));
			...
			if (!$this->form_validation->run()) {
				...
				$this->response['messages'] = array(
                    ...
                    'edit_system_user' => form_error('edit_system_user'),
				...
				if (isset($edit_id) && !empty($edit_id)) {					
					if (is_exist(['email' => $_POST['email']], 'users' , $_POST['edit_system_user'])) {
						$this->response['error'] = true;
						$this->response['message'] = 'Email is already registered.';
						$this->response['csrfName'] = $this->security->get_csrf_token_name();
						$this->response['csrfHash'] = $this->security->get_csrf_hash();
						$this->response['data'] = array();
						print_r(json_encode($this->response));
				...
				$this->response['message'] = (isset($edit_id)) ? ' System User Updated Successfully' : 'System User Added Successfully';
