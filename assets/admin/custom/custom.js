          ...
          $.ajax({
            ...
            success: function (result) {
              ...
              if (currentTotalCount > oldTotalCount) {
                Swal.fire({
                  title: "Total Orders Increased!",
                  text: "New orders have been added.",
                  icon: "success",
...
// partner panel new order popup notification

function getPartnerStoredTotalCount() {
  return parseInt(localStorage.getItem("totalpartnerOrders")) || 0;
}

function updatePartnerStoredTotalCount(newTotalCount) {
  localStorage.setItem("totalpartnerOrders", newTotalCount);
}
...
$(document).ready(function () {
  $.ajax({
    type: "GET",
    url: `${base_url}partner/home/check_partner_login`,
    dataType: "json",

    success: function (data) {
      check_partner_login = data['data'];
      if (check_partner_login == true) {
        setInterval(function () {
          $.ajax({
            type: "GET",
            url: base_url + "partner/orders/view_orders",
            dataType: "json",
            success: function (result) {
              console.log(result);
              
              let currentTotalCount = result["total"];
              let oldTotalCount = getPartnerStoredTotalCount();
              ...
              if (currentTotalCount > oldTotalCount) {
                ...
                updatePartnerStoredTotalCount(currentTotalCount);
              }
            }
          });
        }, 10000);
...
$(document).on("click", "#panel_dark_mode", function () {
  ...
  if ($("body").hasClass("dark-mode")) {
    $("#dark-mode-icon").removeClass("fas fa-moon");
    $("#invoice_bg").removeClass("bg-ligh");
    $("#invoice_bg").addClass("bg-dark");
    $("#dark-mode-icon").addClass("fas fa-sun text-warning");
    ...
    $("#dark-mode-icon").removeClass("fas fa-sun text-warning");
    $("#dark-mode-icon").addClass("fas fa-moon");
    $("#invoice_bg").addClass("bg-ligh");
    $("#invoice_bg").removeClass("bg-dark");
...
function create_editable_variants(
  ...
  if (data[0].variant_ids) {
    ...
    if (
      ...
      // console.log(permuted_value_result);
      ...
      return new Promise((resolve, reject) => {
        $.ajax({
          ...
          success: function (result) {
            if (result["error"] == false) {
              if (status === 7) {
                Swal.fire(`${title}!`, "Partner restored successfully!");
              } else {
                Swal.fire(`${title}!`, "Partner removed successfully!");
              }
              $("table").bootstrapTable("refresh");
            } else {
              Swal.fire("Oops...", result["message"], "error");
...
$(".search_product").each(function () {
  $(this).select2({
    ajax: {
      url: base_url + "admin/product/get_active_product_data",
      dataType: "json",
      delay: 250,
      data: function (data) {
        return {
...
// search offer products

$(".search_offer_product").each(function () {
  $(this).select2({
    ajax: {
      url: base_url + "admin/product/get_offer_products",
      dataType: "json",
      delay: 250,
      data: function (data) {
        return {
...
$(document).on("change", "#promocode_type", function () {
  var promocode_type = $(this).val();
  if (typeof promocode_type != "undefined" && promocode_type == "partner") {
...
$(document).on("click", ".update_authentication_mode", function () {
  setTimeout(function () {
...
$(document).on("click", "#delete-partner-promo-code", function () {
  var id = $(this).data("id");
  Swal.fire({
    title: "Are You Sure !",
    text: "You won't be able to revert this!",
    ...
    preConfirm: function () {
      return new Promise((resolve, reject) => {
        $.ajax({
          url: base_url + "partner/Promo_code/delete_promo_code",
          type: "GET",
          data: { id: id },
          dataType: "json"
        })
...
if (document.getElementById("system-update-dropzone")) {
  var systemDropzone = new Dropzone("#system-update-dropzone", {
    ...
    uploadMultiple: true,
    dictDefaultMessage:
      '<p class="updator-dropzon"><input type="submit" value="Select Files" class="btn btn-info" /><br> or <br> Drag & Drop System Update / Installable / Plugin\'s .zip file Here</p>'
  });
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.error == false) {
        ...
        // Split the error messages by <p> tags
        var errorMessages = result.message.match(/<p>(.*?)<\/p>/g);

        // If there are error messages, loop through and display them
        if (errorMessages) {
          errorMessages.forEach(function (msg) {
            // Remove the <p> and </p> tags and display each error with iziToast
          ...
          // If no <p> tags, show the raw message with iziToast
          iziToast.error({
            message: result.message,
          });
        }
...
$("#partner_table").on("click-cell.bs.table", function (field, value, row, $el) {
...
function save_hours(params) {
  ...
  if ($("input[type=checkbox][name='not_working_days[]']:checked").length) {
    ...
    if (success) {
      iziToast.error({
        message:
          '<span class="messages_style">Please, Select time for working day!</span> '
      });
    }
    if (time_validation) {
      iziToast.error({
        message:
          '<span class="messages_style">Please, Close time can not laser then open time! Set proper time</span> '
      });
    }
  } else {
    iziToast.error({
      message:
        '<span class="messages_style">Please, Select atleast one day for working!</span> '
    });
  }
}
...
$("#save_add_ons").on("click", function (event) {
  ...
  if (!title) {
    iziToast.error({
      message:
        '<span class="messages_style">Please, enter Title for Add Ons!</span> '
    });
    return false;
  }
  if (!price) {
    iziToast.error({
      message:
        '<span class="messages_style">Please, enter Price for Add Ons!</span> '
    });
    return false;
  }
  var is_exist = false;
  $.each(add_on_data, function (i, e) {
    if (e.title === title) {
      iziToast.error({
        message: `<span class="messages_style">${title} is already in list!</span> `
      });
      is_exist = true;
      return false;
    }
  ...
  if (is_exist === false) {
    ...
    iziToast.success({
      message:
        '<span class="messages_style">Add Ons Saved Successfully!</span> '
    });
  }
});
...
$("#save_city").on("click", function (event) {
  ...
  if (vertices && city_id) {
    ...
    iziToast.error({
      message:
        '<span class="messages_style">Please, Select city or its outlines!</span> '
    });
  }
});
...
$(document).on("click", ".edit_attribute", function (e, rows) {
  ...
  if (attribute_values.length == attribute_value_ids.length) {
    ...
    iziToast.error({
      message:
        '<span class="messages_style">Something went wrong!</span> '
    });
  }
});
...
$("#save_charges").on("click", function (event) {
  ...
  if (filter == "") {
    iziToast.error({
      message:
        '<span class="messages_style">Please, YOu can not set blank for range wise delivery charge system.</span> '
    });
    is_error = true;
    return false;
  }
  ...
  for (let i = 0; i < filter.length; i++) {
    if (
      ...
      iziToast.error({
        message:
          '<span class="messages_style">Please, Set proper ranges and prices to setup the range wise charges.</span> '
      });
      is_error = true;
      return false;
    }
  ...
  if (is_error == false) {
    ...
    if (typeof charges != "undefined" && charges !== null) {
      ...
      iziToast.success({
        message: '<span class="messages_style">Charges Saved</span> '
      });
      $("#submit_btn").html("Submit").attr("disabled", false);
    } else {
      iziToast.error({
        message:
          '<span class="messages_style">Please, Enter Proper Range for Charges !</span> '
      });
    }
  }
});
...
function priceFormatter(data) {
  ...
  return (
    '<span class"price_formate_style">' +
    store_currency +
    data
      .map(function (row) {
        return +row[field];
...
$(document).on('change', '.type', function (e, data) {
  ...
  if (sort_type_val == 'cart_notification' && sort_type_val != ' ') {
    $('.cart_notification').removeClass('d-none');
  } else {
    $('.cart_notification').addClass('d-none');
  }
...
$('#erestro_deregister_app').click(function () {
  ...
  if (text == current_code) {
    $.ajax({
      ...
      data: { "app_purchase_code": current_code, },
      dataType: "json",
      success: function (result) {
            ...
            success: function (result) {
              if (result.error == false) {
                ...
                $.ajax({
                  ...
                  data: { 'de_register_app_code': de_register },
                  dataType: 'json',
...
$('#erestro_deregister_web').click(function () {
  ...
  if (text == current_code) {
    $.ajax({
      type: "POST",
      url: base_url + "admin/purchase_code/de_register",
      data: { "web_purchase_code": current_code, },
      dataType: "json",
      success: function (result) {
            ...
            success: function (result) {
              if (result.error == false) {
                ...
                $.ajax({
                  type: 'POST',
                  url: base_url + "admin/purchase_code/delete_purchasecode_data",
                  data: { 'de_register_web_code': de_register },
                  dataType: 'json',
...
$('#delete_selected').click(function () {
  ...
  if (selectedIds.length > 0) {
    if (confirm("Are you sure you want to delete these items?")) {
      ...
      $.ajax({
        url: base_url + from + "/media/delete_bulk",
        type: "POST",
